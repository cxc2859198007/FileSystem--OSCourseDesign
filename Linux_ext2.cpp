...
unsigned int CopyHostToBuffer(string hostpath) {//将要拷贝的文件放到缓冲区中，返回需要的磁盘块数
	//step1: 清空缓冲区
...
void CopyBufferToLinux(unsigned int nowinode) {
	...
	while (!buffer.empty()) { //缓冲区不为空
		...
		inodetb.inode[nowinode].block_pos[inodetb.inode[nowinode].block_num++] = datablock;//更新iNode表
		WriteBlock(datablock + groupdes.data_begin, db); //写入数据块
	}
